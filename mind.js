...
MM.Item = function () {
    ...
    this._dom = {
        ...
        notes: document.createElement("span"), //changed from div to spam to avoid line break
    ...
    this._dom.notes.classList.add("icon-note"); //we need the notes icon to be same as the other icons
...
MM.Item.prototype.mergeWith = function (data) {
    ...
    //todo handle _textColor merging
...
MM.Item.prototype.setTextColor = function (color) {
    this._textColor = color;
    return this.updateSubtree();
}

MM.Item.prototype.getTextColor = function () {
    return this._textColor || (this.isRoot() ? MM.Item.COLOR : this._parent.getTextColor());
}
...
MM.Item.prototype._updateTextColor = function () {
    this._dom.content.style.color = this._textColor;
}
MM.Item.prototype._updateNotesIndicator = function () {
    let icon = "fa-commenting-o";
    if (this._notes) {
        this._dom.notes.classList.add('fa');
        this._dom.notes.classList.add(icon);
    } else {
        this._dom.notes.classList.remove('fa');
        this._dom.notes.classList.remove(icon);
        // this._dom.notes.style.display = "none";
    }
    // if (this._notes)
    // {
    //     this._dom.notes.classList.add("icon");
...
MM.Map = function (options) {
    var o = {
        root: "Topic name",
        layout: MM.Layout.Graph.Right //MM.Layout.Map 
    }
...
MM.Map.prototype.getItemFor = function (node, refObj) {
    if (isReadOnly) {
        blur();
    }
    var port = this._root.getDOM().node.parentNode;
    if (node.classList.value == "icon-note fa fa-commenting-o") {
        if (refObj) {
            refObj.openNote = true;
        }
    }
...
MM.Keyboard.stopListening = function () {
...
MM.Action.SetTextColor = function (item, color) {
    if (MM.App.isReadOnly) {
        return;
    }
    this._item = item;
    this._textColor = color;
    this._oldTextColor = item.getOwnTextColor();
}
...
MM.Clipboard.focus = function () {
    if (isMobile) {
        this._node.blur(); //fixed issue of keyboard poping in mobile screen
    } else {
        this._node.focus();
    }
    this._empty();
...
MM.Command.InsertImage = Object.create(MM.Command, {
    label: { value: "Add Image" },
    keys: { value: [{ keyCode: "K".charCodeAt(0), ctrlKey: true }] }
});
MM.Command.InsertImage.execute = function () {

    var element = document.getElementById("InsertImage")
...
function uploadFile(fileData) {
    ...
    var parsed = $.parseHTML(MM.App.map.getName());
    if (parsed.length > 0) {
        formData.append("mapName", parsed[0].data);
    } else {
        formData.append("mapName", MM.App.map.getName());
    }
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            MM.App.current.setText(img.outerHTML)
        },
...
MM.Command.NewChildMap = Object.create(MM.Command, {
    label: {
        value: "New map"
    },
    keys: {
...
MM.Command.NewChildMap.execute = function (e) {
    $("#loadChildsId").find(".current").eq(0).addClass("btn-default")
    $("#loadChildsId button").removeClass("current btn-primary"); //remove current class if button has..

    if ($.inArray("tablinks", e.target.classList) > -1) {//if you're about to add a child map from tabs, then first save current map
        MM.App.io.quickSave();
    }

    var map = new MM.Map();
    if (location.search.indexOf("url") > 0) {//map is already saved
        var parentMapId = decodeURIComponent(location.search).split('-pid=')[1];
    } else {
        //first save map
        MM.UI.Backend.WebDAV.save();
    }
    if (parentMapId == "undefined" || parentMapId == "") {
        parentMapId = MM.App.map.getId();
    }
    MM.App.setMap(map);
    MM.App.map.parentMapId = parentMapId;

    //save new child map
    MM.UI.Backend.WebDAV.save(true);
    var mapBtn = {
        Url: MM.App.map.getName() + "-id=" + MM.App.map.getId() + "-pid=" + parentMapId,
        DisplayName: MM.App.map.getName()  
    }
    setChildTab(mapBtn);
};

MM.Command.LoadChildMap = Object.create(MM.Command, {
    label: {
        value: "Load child map"
    },
    keys: {
        value: [{
            keyCode: "N".charCodeAt(0),
...
MM.Command.LoadChildMap.execute = function (e) {
    if ($.inArray("tablinks", e.target.classList) > -1) {//if you're about to load a child map from tabs, then first save current map
        MM.App.io.quickSave();
    }

    MM.UI.Backend.WebDAV._load(e.target.getAttribute('url'));
};
...
MM.Command.FinishAndAddChild = Object.create(MM.Command, {
    keys: {
        value: [{ keyCode: 9, altKey: false, ctrlKey: false, shiftKey: false }]
    },
    editMode: { value: true }
});
MM.Command.FinishAndAddChild.execute = function () {
    MM.Command.Finish.execute();
...
MM.Layout = Object.create(MM.Repo, {
    ALL: { value: [] },
    SPACING_RANK: { value: 15 },
    SPACING_CHILD: { value: 15 },
});
...
MM.Backend.WebDAV._request = function (method, url, data) {
    ...
    //}
    if (method == "get") {
        url = root + "/Home/LoadMap" + url;
    } else if (method == "post" || method == "put") {
        url = root + "/Home/UpdateMap" + url;
    }

    if (data && data.length > 2094200) {
        toastr.error("You have reached the max limit of a map, please delete some nodes and then create child maps from bottom left '+' icon");
        MM.App.setThrobber(false);
        return;
    }else if (data && data.length > 1990000) {
        toastr.warning("You're about to reach the max limit of a map, try creating child maps from bottom left '+' icon");
    } 
    ...
    Promise.send(xhr, data).then(
        function (xhr) {
            if (JSON.parse(xhr.responseText) == "Session Expired") {
                try {
                    var win = window.open('/account/login?message=session_expired_while_saving', '_blank');
                    win.focus();
                } catch (ex) {
                    toastr.warning("Session is Expired, please login and then hit save again. Allow popup to open so that you can login without loosing changes");
                }
            } else {
...
MM.Backend.Firebase.connect = function (server, auth) {
    ...
    var config = {
        ...
        databaseURL: "https://boilerplatedev-52f7d-default-rtdb.firebaseio.com",
    ...
    //below function gets all maps and creates a list
...
MM.UI.prototype._update = function () {
    if (isReadOnly) {
        //toastr.warning("You cannot make changes in a read-only map, please login");
        return;
    }
    this._layout.update();
    this._shape.update();
...
MM.UI.TextColor = function () {
    this._node = document.querySelector("#textColor");
    this._node.addEventListener("click", this);
...
MM.UI.TextColor.prototype.handleEvent = function (e) {
    ...
    var color = e.target.getAttribute("data-color") || null;
    var action = new MM.Action.SetTextColor(MM.App.current, color);
    MM.App.action(action);
}
...
MM.UI.Help.prototype.close = function () { //do we need this function? its added from notes pull-request
    if (this._node.classList.contains("visible")) {
        this._node.classList.toggle("visible");
    }
}
...
MM.UI.Notes.prototype.toggle = function () {
    ...
    if (this._node.classList.contains("visible"))
        hideChildMaps();
    //MM.UI.IO._isNoteOpen = true;
    else
        makeChildMapsVisible();
        //MM.UI.IO._isNoteOpen = false;
}
...
MM.UI.IO.prototype.restore = function () {
    var parts = {};
    decodeURIComponent(location.search).substring(1).split("&").forEach(function (item) {
        var keyvalue = item.split("=");
        parts[decodeURIComponent(keyvalue[0])] = decodeURIComponent(keyvalue[1] + "=" + keyvalue[2] + "=" + keyvalue[3]);
    });
    ...
    parts.b = "webdav";
    if (parts.url && parts.url.indexOf("undefined") > -1)
        parts.url = decodeURIComponent(window.location.search.split("=")[1]);
...
MM.UI.Backend._loadDone = function (json) {
    ...
    try {
        ...
        MM.App.map.parentMapId = location.search.split('-pid=')[1];
...
//list parameter: list of all maps
//select paramter: where to append list (html tag)
MM.UI.Backend._buildList = function (list, select) {
    ...
    //display a list of existing mind maps
    showExistingMaps(data);
}
...
MM.UI.Backend.WebDAV.save = function (isChildMap) {
    if (isChildMap) {
    ...
    if (!map.parentMapId) {
        map.parentMapId = decodeURIComponent(location.search).split('-pid=')[1];
    }
    url = url + "-id=" + map.getId() + "-pid=" + map.parentMapId;
    this._current = url;
    var json = map.toJSON();
    var data = MM.Format.JSON.to(json);

    var parsed = $.parseHTML(map.getName());
    if (parsed.length > 0) {
        $('#loadChildsId').find('.current').eq(0).text(parsed[0].data);
    }
    else {
        $('#loadChildsId').find('.current').eq(0).text(map.getName());
    }
...
MM.UI.Backend.WebDAV._loadDone = function (data) {
    try {
        isNewMap = false;
        obj = JSON.parse(data);
        mapCreaterId = obj.postedBy;
        loadDone = true;
        var json = MM.Format.JSON.from(obj.content);
        refreshShowHide();
        
        MM.UI.Backend._loadDone.call(this, json);
        if (obj.childMaps) {
            setTimeout(showChildMaps(obj.childMaps), 500);
        }
    } catch (e) {
        this._error(e);
    }
...
MM.UI.Backend.Firebase.init = function (select) {
    ...
    this._server.value = localStorage.getItem(this._prefix + "server") || "server name";
...
MM.Mouse.init = function (port) {
    ...
    this._port.addEventListener("contextmenu", this); //right click event
}

MM.Mouse.handleEvent = function (e) {
    switch (e.type) {
        case "click":
            let refObj = { openNote: false };
            var item = MM.App.map.getItemFor(e.target, refObj);
            if (MM.App.editing && item == MM.App.current) { return; } /* ignore on edited node */
            if (refObj.openNote) {
                MM.Command.Notes.execute();
            }
        ...
        switch (message) {
            ...
            case "item-change":
                if (publisher.isRoot() && publisher.getMap() == this.map) {
                    var parsed = $.parseHTML(this.map.getName());
                    if (parsed.length > 0) {
                        document.title = parsed[0].data + " - " + CONST_APP_NAME;
                    } else {
                        document.title = this.map.getName() + " - " + CONST_APP_NAME;
                    }
                }
                break;
        }
